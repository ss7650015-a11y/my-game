...
function createLevel(number) {
  ...
  // Very long stages: about 72,000 px and increasing, targeting ~3 minutes of smooth play per stage.
  const width =
    72000 + (number - 1) * 1000;
  ...
  if (number >= 5) {
    for (let i = 2; i < list.length - 1; i += 3) {
      const p = list[i];
      generatedEnemies.push(
        enemy(
          ((i + number + 4) % 7) + 1,
...
function drawLevelHazards() {
  ...
  for (const h of hazards) {
    ...
    g.addColorStop(0, '#19a7d8');
    g.addColorStop(1, '#075a93');
    ctx.fillStyle = g;
    ctx.fillRect(x, y, w, h.height);

    ctx.strokeStyle = 'rgba(220,250,255,.8)';
    ctx.lineWidth = 2;
    ...
    const bob = Math.sin(performance.now()*0.003 + h.phase) * 5;
    const cx = x + w * (0.45 + 0.12*Math.sin(h.phase));

    if (h.kind === 'whale') {
      // Whale surfacing
      ctx.fillStyle='#355f91';
      ctx.beginPath();
      ctx.ellipse(cx, y + 18 + bob, 42, 17, 0, 0, Math.PI*2);
      ctx.fill();
      ctx.fillStyle='#4f7fb0';
      ctx.beginPath(); ctx.ellipse(cx-12,y+13+bob,16,7,0,0,Math.PI*2); ctx.fill();
      ctx.fillStyle='#355f91';
      ctx.beginPath(); ctx.moveTo(cx-28,y+12+bob); ctx.quadraticCurveTo(cx-44,y-4+bob,cx-34,y-15+bob); ctx.quadraticCurveTo(cx-25,y-5+bob,cx-18,y+10+bob); ctx.fill();
      ctx.fillStyle='#fff'; ctx.beginPath(); ctx.arc(cx+27,y+13+bob,3,0,Math.PI*2); ctx.fill();
      ctx.fillStyle='#111'; ctx.beginPath(); ctx.arc(cx+28,y+13+bob,1.5,0,Math.PI*2); ctx.fill();
      ctx.strokeStyle='rgba(255,255,255,.7)'; ctx.lineWidth=3; ctx.beginPath(); ctx.moveTo(cx-2,y+2+bob); ctx.quadraticCurveTo(cx-5,y-18+bob,cx+2,y-28+bob); ctx.stroke();
    } else {
      // Crocodile snout and eyes
      ctx.fillStyle='#4b8b42';
      ctx.beginPath(); ctx.roundRect(cx-52,y+8+bob,104,23,9); ctx.fill();
      ctx.fillStyle='#79b65d';
      ctx.beginPath(); ctx.ellipse(cx-26,y+8+bob,13,9,0,0,Math.PI*2); ctx.ellipse(cx+26,y+8+bob,13,9,0,0,Math.PI*2); ctx.fill();
      ctx.fillStyle='#fff'; ctx.beginPath(); ctx.arc(cx-24,y+5+bob,4,0,Math.PI*2); ctx.arc(cx+24,y+5+bob,4,0,Math.PI*2); ctx.fill();
      ctx.fillStyle='#111'; ctx.beginPath(); ctx.arc(cx-24,y+5+bob,2,0,Math.PI*2); ctx.arc(cx+24,y+5+bob,2,0,Math.PI*2); ctx.fill();
      ctx.fillStyle='#fffde7';
      for(let tooth=-3; tooth<=3; tooth++){ ctx.beginPath(); ctx.moveTo(cx+tooth*12,y+27+bob); ctx.lineTo(cx+tooth*12+4,y+27+bob); ctx.lineTo(cx+tooth*12+2,y+34+bob); ctx.closePath(); ctx.fill(); }
    }
  }
}
...
(function fixAccidentalLevelSuffix() {
  function cleanLevelLabel(root) {
    ...
    for (const textNode of nodes) {
      ...
      // Only touch a level label that actually ends in /15/07.
...
/* ============================================================
   SAFE CHECKPOINT SAVE SYSTEM
   Adds ONLY:
   - Red flower checkpoints at 25%, 50%, 75% of every stage.
   - Save when the player collects a checkpoint flower.
   - Save stage completion at the goal.
   - Load Game button on the main menu.
   Existing gameplay, enemies, HUD, menu and visuals are not replaced.
   ============================================================ */
(function installCheckpointSaveSystem() {
  ...
  const SAVE_KEY = "naughtyBoyCheckpointSaveV1";
  const CHECKPOINTS = [0.25, 0.50, 0.75];
  let flowers = [];
  let saveData = null;
  let originalDraw = null;
  let originalLoadLevel = null;
  let originalNextLevel = null;
  let originalSetLanguage = null;
  ...
  function getLevelWidth() {
    return Number(currentLevelWidth || 0) || 1;
  }
  ...
  function rebuildFlowers() {
    flowers = [];
    const width = getLevelWidth();
    for (let i = 0; i < CHECKPOINTS.length; i++) {
      const x = Math.max(140, Math.min(width - 140, width * CHECKPOINTS[i]));
      const p = findPlatformAt(x);
      const groundY = p
        ? p.y
        : Math.max(0, (typeof canvas !== "undefined" ? canvas.height : 600) - 95);
      flowers.push({
        index: i + 1,
        x,
        y: groundY - 48,
    ...
    if (saveData && Number(saveData.level) === Number(currentLevel)) {
      const cp = Number(saveData.checkpoint || 0);
      flowers.forEach(f => {
        f.collected = f.index <= cp;
      });
    }
  }

  function flowerHit(f) {
    if (!f || f.collected || !player) return false;
    return intersects(
      player,
      { x: f.x - 22, y: f.y - 42, width: 44, height: 52 }
    );
  }

  function saveCheckpoint(checkpointIndex) {
    const old = readSave() || {};
    const level = Number(currentLevel) || 1;
    const oldLevel = Number(old.level) || 1;
    const oldCheckpoint = oldLevel === level ? Number(old.checkpoint || 0) : 0;

    const data = {
      level: Math.max(oldLevel, level),
      checkpoint: level > oldLevel ? checkpointIndex : Math.max(oldCheckpoint, checkpointIndex),
      x: Number(player && player.x) || 100,
      y: Number(player && player.y) || 400,
    ...
    if (typeof announce === "function") {
      ...
      announce(en ? "🌺 Game saved!" : "🌺 تم حفظ اللعبة!");
    }
  }

  function saveStageCompleted(levelNumber) {
    const old = readSave() || {};
    const data = {
      level: Math.max(Number(old.level) || 1, Number(levelNumber) || 1),
      checkpoint: 3,
      x: Number(player && player.x) || 100,
      y: Number(player && player.y) || 400,
      cameraX: Number(cameraX) || 0,
    ...
    data.completedLevels[String(levelNumber)] = true;
    writeSave(data);
  }
  ...
  function restoreSave() {
    ...
    if (!data) {
      const en = typeof selectedLanguage !== "undefined" && selectedLanguage === "en";
      alert(en ? "There is no saved game yet." : "لا يوجد حفظ للعبة حتى الآن.");
      return false;
    }

    const level = Math.max(1, Math.min(
      typeof TOTAL_LEVELS !== "undefined" ? TOTAL_LEVELS : 15,
      Number(data.level) || 1
    ));
    ...
    lives = Math.max(1, Number(data.lives) || 3);
    gameOver = false;
    gameWon = false;
    changingLevel = false;
    gameRunning = false;
    ...
    const cp = Math.max(0, Math.min(3, Number(data.checkpoint) || 0));
    if (cp > 0 && player) {
      const target = cp >= 3
        ? null
        : flowers[cp - 1];
      if (target) {
        player.x = target.x;
        const p = findPlatformAt(target.x);
        if (p) player.y = p.y - player.height;
      } else if (Number(data.x)) {
        player.x = Number(data.x);
        player.y = Number(data.y) || player.y;
      }
      cameraX = Math.max(0, Number(data.cameraX) || Math.max(0, player.x - canvas.width * 0.35));
    }

    updateHUD();
    if (typeof message !== "undefined" && message) message.classList.add("hidden");
  ...
  function addLoadButton() {
    if (!startMenu) return;
    const card = startMenu.querySelector(".play-btn")?.parentElement;
    if (!card) return;
    if (card.querySelector(".load-save-btn")) return;
    ...
    btn.style.cssText = `
      ...
      border:0;border-radius:16px;background:#27ae60;color:#fff;
      font-size:24px;font-weight:800;cursor:pointer;
      box-shadow:0 8px 0 #176b3a;
    `;
    btn.addEventListener("click", restoreSave);
  ...
  function updateLoadButtonText() {
    if (!startMenu) return;
    const btn = startMenu.querySelector(".load-save-btn");
    if (!btn) return;
    const en = typeof selectedLanguage !== "undefined" && selectedLanguage === "en";
    btn.textContent = en ? "▶ LOAD GAME" : "▶ تحميل الحفظ";
  }

  function drawFlower(f) {
    if (!f || typeof ctx === "undefined" || typeof cameraX === "undefined") return;
    const x = f.x - cameraX;
    const y = f.y;
    if (x < -70 || x > canvas.width + 70) return;
    ...
    ctx.beginPath();
    ctx.ellipse(-8, 19, 9, 5, -0.45, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.ellipse(8, 12, 9, 5, 0.45, 0, Math.PI * 2);
    ctx.fill();

    // Red flower.
    ctx.fillStyle = f.collected ? "#8e8e8e" : "#e53935";
    for (let i = 0; i < 5; i++) {
      const a = (Math.PI * 2 * i) / 5;
      ctx.beginPath();
      ctx.arc(Math.cos(a) * 10, Math.sin(a) * 10, 9, 0, Math.PI * 2);
      ctx.fill();
    }
    ...
    // Small save mark above an uncollected flower.
    if (!f.collected) {
  ...
  function updateCheckpointSystem() {
    if (!gameRunning || !player) return;
    for (const f of flowers) {
      if (flowerHit(f)) {
        f.collected = true;
        saveCheckpoint(f.index);
        break;
      }
    }
  }

  // The original draw is preserved; we only draw the new flowers afterward.
  originalDraw = typeof window.draw === "function" ? window.draw : null;
  if (originalDraw && !originalDraw.__checkpointWrapped) {
  ...
  // Rebuild flowers after every real level load, without changing level creation.
  originalLoadLevel = typeof window.loadLevel === "function" ? window.loadLevel : null;
  if (originalLoadLevel && !originalLoadLevel.__checkpointWrapped) {
    const wrappedLoadLevel = function(...args) {
      const result = originalLoadLevel.apply(this, args);
      saveData = readSave();
      rebuildFlowers();
      return result;
    };
    wrappedLoadLevel.__checkpointWrapped = true;
    window.loadLevel = wrappedLoadLevel;
  }

  // Save stage completion before the original next-level transition.
  originalNextLevel = typeof window.nextLevel === "function" ? window.nextLevel : null;
  if (originalNextLevel && !originalNextLevel.__checkpointWrapped) {
    const wrappedNextLevel = function(...args) {
      const completed = Number(currentLevel) || 1;
      saveStageCompleted(completed);
      const result = originalNextLevel.apply(this, args);

      // If another stage was opened, keep the save ready to continue there.
      // The completed stage remains recorded in completedLevels.
      if (!gameWon && Number(currentLevel) !== completed) {
        const old = readSave() || {};
        writeSave({
          level: Number(currentLevel) || completed,
          checkpoint: 0,
          x: 100,
          y: 400,
          cameraX: 0,
    ...
    window.nextLevel = wrappedNextLevel;
  }

  // Keep the Load Game label translated when language changes.
  originalSetLanguage = typeof window.setLanguage === "function" ? window.setLanguage : null;
  if (originalSetLanguage && !originalSetLanguage.__checkpointWrapped) {
    const wrappedSetLanguage = function(...args) {
      const result = originalSetLanguage.apply(this, args);
      updateLoadButtonText();
      return result;
    };
    wrappedSetLanguage.__checkpointWrapped = true;
    window.setLanguage = wrappedSetLanguage;
  }

  // The stable script has already created the main menu at this point.
  addLoadButton();
  saveData = readSave();
  rebuildFlowers();

  // Lightweight checkpoint collision check. No new game loop is created.
  setInterval(updateCheckpointSystem, 50);

  // Redraw once so the checkpoint flowers are visible immediately.
  if (typeof window.draw === "function" && typeof gameRunning !== "undefined" && !gameRunning) {
    window.draw();
  }
})();
